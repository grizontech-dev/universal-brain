...
vi.mock("../../../src/services/analytics.service.js", () => ({
  analyticsService: {
    ...
    getModelDistribution: vi.fn(async () => []),
...
describe("analytics admin routes (integration)", () => {
  beforeAll(async () => {
  ...
  it("GET /api/v1/admin/analytics/overview requires admin auth", async () => {
    const app = buildApp();
    const response = await request(app).get("/api/v1/admin/analytics/overview").set("x-platform", "admin");
